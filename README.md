...
- **One host-observation boundary on both OSes** — the agent invokes the Nix-baked [`osfacts` V2](https://github.com/juspay/kolu/pull/2011) binary through its version-gated TypeScript client. One atomic subtree snapshot supplies process identity, RSS, start time, and the complete claimed/unclaimed TCP listener table; one `host` reading supplies load, memory, swap, uptime, cumulative per-core CPU, network counters, and root-disk capacity. Drishti does not search `PATH` or fall back to `lsof`, `ps`, `/proc`, `sysctl`, `vm_stat`, `netstat`, or `statfs`.
- **Click a process for details** — selecting a row opens an inline panel with the process name, linked parent PID, RSS, start time, attributed listeners, and facet-specific inspection status. Permission-blind sockets remain visible in a separate unclaimed-listener strip (with UID on Linux when available), rather than becoming a falsely empty process port list. Click the row again, the `✕`, or press `Esc` to close.
...
Host 1: drishti-agent     Host 2: drishti-agent     …
   │  osfacts V2 on linux + darwin (all host/process/socket observations)
   ▼
Kernel
```
...
| **Collection** | `processes` | Keyed by PID — `{ command, ppid, rssBytes, startedAtMs, listeners, unreadable[] }`, populated from `snapshot --roots 1 --procs --ports --mem --start-time`. `unreadable` identifies the exact `proc`, `ports`, `mem`, or `start_time` facet; a `U`-only pid remains represented. Snapshot-then-delta. |
