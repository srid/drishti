...
- **One process/socket fact path on both OSes** — the agent invokes the Nix-baked [`osfacts`](https://github.com/juspay/kolu/pull/1996) binary through its version-gated TypeScript client. The process table shows PID, parent PID, name, and attributed listening TCP ports from one atomic `snapshot --roots 1 --procs --ports`; it does not search `PATH` or fall back to platform-specific inspection. Per-process RSS and age await osfacts' planned OSF3 facets and are deliberately not fabricated meanwhile; host-level CPU and memory telemetry remains available.
- **Click a process for details** — selecting a row opens an inline panel with the process name, linked parent PID, each listener's raw network-order bind address and port, and inspection status. osfacts' mandatory `U` rows are first-class: `readable` plus no listeners means observed-empty, while `blind · EACCES`/`EPERM` means the empty listener set is not authoritative. Click the row again, the `✕`, or press `Esc` to close.
...
Host 1: drishti-agent     Host 2: drishti-agent     …
   │  osfacts on linux + darwin (processes/sockets); platform host telemetry
   ▼
Kernel
```
...
| **Collection** | `processes` | Keyed by PID — `{ command, ppid, listeners: [{ port, address }], unreadableErrno }`, populated on linux and darwin through the dependency-free `osfacts-client` over the binary baked at `DRISHTI_OSFACTS_BIN`. The client requests PID 1's subtree with `--procs --ports`, refuses an unknown TSV schema version, and never searches `PATH` or falls back to another OS reader. A pid reported only by a mandatory osfacts `U` row still appears in this collection. `listeners: []` + `unreadableErrno: null` is observed-empty; the same array with an errno is blind, never silently empty. Per-process memory/start-time are intentionally absent until osfacts OSF3 supplies those facets. Snapshot-then-delta. |
