/**
 * Pure metric-history domain — framework-free ring maths shared by the
 * **agent daemon** (which owns the durable ring on disk and samples every poll
 * tick), the **parent** (which re-serves the ring to browsers), and the
 * **browser** (which renders the ring it's streamed).
 *
 * The ring lives in the agent daemon under `~/.local/state/drishti/` and
 * survives parent deploys / reconnects. A browser still gets the whole ring
 * on connect (snapshot) then per-tick deltas. Corrupt or unknown-version
 * on-disk rings surface as a typed `unavailable` disposition — never a
